...
### First steps 
```{r}
plot.state <- function(df,enrollment_value){

  
  # Filter out the ERROR setting of Division
  df1 <- df %>%
    filter(division != "ERROR") %>%
    mutate(divisionf = as.factor(division))
  
  # Calculate the mean of the specified variable for each combination of year and division
  summary_df <- df1 %>%
