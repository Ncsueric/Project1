...
library(ggplot2)
library(httr)
# Read csv or handle error when www4.stat.ncsu.edu is down.
read_csv_safe <- function(url, max_retries = 3, timeout = 20) {
  for (i in seq_len(max_retries)) {
    ...
    if (!is.null(response) && http_status(response)$category == "Success") {
      return(read_csv(content(response, "text"), show_col_types = FALSE))
...
EDU <- read_csv_safe("https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv") |>
